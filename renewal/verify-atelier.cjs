...
try {
 ...
 const metadata=await page.evaluate(async()=>{const v=document.createElement('video');v.src='.test-output/atelier/two-photos.mp4';await new Promise((r,j)=>{v.onloadedmetadata=r;v.onerror=j;});return {width:v.videoWidth,height:v.videoHeight,duration:v.duration};});
