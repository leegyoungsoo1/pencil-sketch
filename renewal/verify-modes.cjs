...
module.exports=async({chromium,server,root})=>{
 ...
 try{
  ...
  await page.goto('http://127.0.0.1:'+server.address().port+'/classic.html');
  assert.equal(await page.inputValue('#programMode'),'classic.html');
  ...
  await Promise.all([page.waitForURL('**/classic.html'),page.selectOption('#programMode','classic.html')]);
  assert.equal(await page.inputValue('#seconds'),'20');
  assert.equal(await page.inputValue('#style'),'ai');
  assert(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));
