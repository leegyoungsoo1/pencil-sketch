...
module.exports=async({chromium,server,root})=>{
 const out=path.join(root,'.test-output/sweeps');fs.mkdirSync(out,{recursive:true});await new Promise(r=>server.listen(0,'127.0.0.1',r));const browser=await chromium.launch({headless:true,channel:'msedge'});
 try{
 ...
 });console.log(metrics);assert.equal(metrics.deposited,metrics.source);assert.equal(metrics.unique,metrics.source);assert(metrics.minDuration>=87);assert(metrics.distance<16);assert(metrics.total>20000);assert(metrics.summary.includes('자동'));
 ...
 const motion=await page.evaluate(()=>{const sweep=plan.strokes.find(s=>s.inkGroups&&s.len>25),samples=[];for(let t=sweep.tDown;t<sweep.tUp;t+=1000/30){renderFrame(t);const p=pencilAt(t);samples.push({time:t,x:p.x,y:p.y});}return samples;});assert(motion.length>=4);console.log('visible sweep samples',motion.length);
 ...
 });assert(writing.extension>0);assert.equal(writing.start,17000+writing.extension);assert.equal(writing.drawEnd,16800+writing.extension);assert.equal(writing.outro,2500);assert.equal(writing.talkStart,writing.start);assert.equal(writing.beat,17050+writing.extension);
 ...
 const frames=[];for(const seconds of [0,5,12,20,35,60]){await page.evaluate(t=>renderFrame(t*1000),seconds);const name=`frame-${seconds}.jpg`;fs.writeFileSync(path.join(out,name),Buffer.from(await page.evaluate(()=>canvas.toDataURL('image/jpeg',.9).split(',')[1]),'base64'));frames.push({name,seconds});}
