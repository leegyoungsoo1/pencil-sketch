...
module.exports=async({chromium,server,root})=>{
 ...
 const metrics=await page.evaluate(()=>{
  ...
  return {total:plan.totalMs,sweeps:sweeps.length,deposited:marks.length,source:StudioGraphite.marks(analysis).filter(s=>s.kind!=='contour').length,unique:new Set(marks.map(m=>m.sourceMark)).size,minDuration,distance,summary:document.querySelector('#projectSummary').textContent};
 });console.log(metrics);assert.equal(metrics.deposited,metrics.source*4);assert.equal(metrics.unique,metrics.source);assert(metrics.distance<20);assert(Math.abs(metrics.total-120000)<1);assert(metrics.summary.includes('2분'));
 const shortTouches=await page.evaluate(()=>{const strokes=plan.strokes.filter(s=>s.inkGroups);return {mark:Math.max(...StudioGraphite.marks(analysis).filter(s=>s.kind==='hatch').map(s=>s.len)),span:strokes.reduce((max,s)=>Math.max(max,Math.hypot(Math.max(...s.x)-Math.min(...s.x),Math.max(...s.y)-Math.min(...s.y))),0)};});console.log('short touches',shortTouches);assert(shortTouches.mark<=6);assert(shortTouches.span<18);
 const regions=await page.evaluate(()=>{const list=StudioGraphite.marks(analysis).filter(s=>s.kind==='hatch'),labels=StudioGraphite.regions(analysis),bins=new Set();let crossing=0;for(const s of list){bins.add(Math.round(Math.atan2(s.y[2]-s.y[0],s.x[2]-s.x[0])*4));for(let i=0;i<s.n;i++){const x=Math.round(s.x[i]-analysis.b.x),y=Math.round(s.y[i]-analysis.b.y);if(!labels[y*analysis.gw+x])crossing++;}}return {crossing,directions:bins.size};});console.log('regions',regions);assert.equal(regions.crossing,0);assert(regions.directions>8);
 ...
 });console.log('fidelity',fidelity);assert(fidelity.mean<3);
 ...
 const layers=await page.evaluate(()=>{
  const strokes=plan.strokes.filter(s=>s.inkGroups),counts=new Map();let weak=true,ordered=true;
  for(const s of strokes){for(const m of s.inkGroups.flat()){weak&&=m.alpha<m.sourceMark.alpha;counts.set(m.sourceMark,(counts.get(m.sourceMark)||0)+1);}}
  for(let i=0;i<strokes.length;i+=4)for(let j=0;j<4;j++)ordered&&=strokes[i+j].layer===j&&strokes[i+j].patch===strokes[i].patch;
  return {weak,ordered,four:[...counts.values()].every(n=>n===4)};
 });assert(layers.weak&&layers.ordered&&layers.four);console.log('PASS: each neighbourhood receives four faint layers before moving on');
 ...
 await page.selectOption('#framing','scene');await page.click('#createArtwork');await page.waitForFunction(()=>plan&&analysis?.scene&&!photoInput.disabled,null,{timeout:240000});
 const scene=await page.evaluate(()=>{const m=StudioGraphite.marks(analysis).filter(s=>s.kind==='hatch'),f=analysis.face;return {full:analysis.subject.every(v=>v===1),outer:m.filter(s=>Math.hypot((s.x[1]-analysis.b.x-f.x)/f.rx,(s.y[1]-analysis.b.y-f.y)/f.ry)>2).length,marks:m.length};});assert(scene.full&&scene.outer>100);console.log('PASS: landscape keeps background outside face',scene);
 await page.evaluate(()=>renderFrame(plan.totalMs));fs.writeFileSync(path.join(out,'landscape-mode.jpg'),Buffer.from(await page.evaluate(()=>canvas.toDataURL('image/jpeg',.9).split(',')[1]),'base64'));
